...
def play_game(iterations, game_class, player_classes, models=[None,None], deck = None, hide_cards=[False, False]):
    """
    ...
    for i in range(iterations):
        ...
        while not check_win(players, game):
            ...
            if len(available_routes) == 0 or attacker_player.draw_or_claim(game, players, hide_cards[0]) == 0:
                cards = game.draw_cards(attacker_player)
                actions.append(cards)
            else:
                route = attacker_player.choose_route(game, players, hide_cards[0])
            ...
            available_routes = get_available_routes(availability)
            if len(available_routes) == 0 or defender_player.draw_or_claim(game, players[::-1], hide_cards[1]) == 0:
                cards = game.draw_cards(defender_player)
                actions.append(cards)
            else:
                route = defender_player.choose_route(game, players[::-1], hide_cards[1])
...
if __name__ == '__main__':
    attacker_net = load_net("selfplay_model.pth.tar", 874, PGNetwork)
    rewards, records = play_game(1000, Game, [PGPlayer, GreedyPlayer], models=[attacker_net, attacker_net], deck=None, hide_cards=(False, True))
    print(sum(rewards[0]), sum(rewards[1]))
    with open("PGSelfplayHideTraining.json", "w") as outfile:
