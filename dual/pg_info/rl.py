...
def rollout_attacker(models, game_class, player_classes, deck=None, MAX_T=100, gamma=0.9):
    ...
    while T < MAX_T:
        ...
        dist = models[0](compute_input_matrix(game, players, hide_cards=True),mask)
        action = dist.sample()
        ...
        available_routes = get_available_routes(availability)
        if len(available_routes) == 0 or nonrl_player.draw_or_claim(game, players,hide_cards=False) == 0:
            cards = game.draw_cards(nonrl_player)
        else:
            route = nonrl_player.choose_route(game, players, hide_cards=False)
...
def rollout_defender(models, game_class, player_classes, deck=None, MAX_T=100, gamma=0.9):
    ...
    while T < MAX_T:
        ...
        if len(available_routes) == 0:
            ...
            mask = torch.cat([torch.tensor([[1]]), torch.reshape(torch.from_numpy(availability), (7*7*7, 1))])
        dist = models[1](compute_input_matrix(game, players, hide_cards=True),mask, )
        action = dist.sample()
        log_prob = dist.log_prob(action)
        if action == 0:
            cards = game.draw_cards(pg_player)
...
if __name__ == '__main__':
    initial_model = None
    checkpoint_file ="supervised_model.pth.tar"
    reward_file = "supervised_reward.pdf"
    ...
    for step in range(20000):
        if step % 2 == 0:
            actions, rewards, log_probs, values, ep_reward = rollout_defender([policy_net, policy_net], Game, [GreedyPlayer, PGPlayer], deck=None)
        else:
            actions, rewards, log_probs, values, ep_reward = rollout_attacker([policy_net, policy_net], Game, [PGPlayer, GreedyPlayer], deck=None)
