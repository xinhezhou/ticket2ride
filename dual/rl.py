...
def rollout_attacker(models, game_class, player_classes, deck=None, MAX_T=100, gamma=0.9):
    ...
    while T < MAX_T:
        ...
        action = dist.sample()
        # print(action)
        # valid_dist = torch.mul(torch.transpose(dist, 0, 1)[1:], availability)
        # action = valid_dist.sample()
        ...
        if action == 0:
            ...
            # print(u,v,c)
            if u >= v:
                print("here", (u,v,c), (mask == 0)[action], log_prob)
...
if __name__ == '__main__':
    ...
    attacker_reward_file = "pg_pg_attacker_reward.pdf"
    attacker_net = load_net(initial_defender, 874, PGNetwork)
    defender_net = load_net(initial_attacker, 874, PGNetwork)
